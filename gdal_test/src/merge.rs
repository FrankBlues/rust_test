...
pub fn merge<T: AsRef<OsStr>>(
    ...
    let out_metas = output_merge_metas(&files, output_res, output_nodata, output_count, output_bounds)?;
    let dtype = out_metas.dtype;
    match dtype {
        1 => {
            type U = u8; //  GDALDataType::GDT_Byte
            return merge_files::<_, U>(files, output_file, &out_metas, resample_method, output_band_index);
        }
        2 => {
...
pub fn output_merge_metas<T: AsRef<OsStr>>(
    ...
    let dst_w; let dst_s; let dst_e; let dst_n;
    match output_bounds {
        Some(b) => {
            dst_w = b[0];
            dst_s = b[1];
...
pub fn merge_files<T: AsRef<OsStr>, U: Copy + GdalType>(
    ...
    let driver = Driver::get(&out_metas.driver).unwrap();
    // let mut output_ds = driver.create("/data/mosaic.tif", output_width, output_height, out_count).unwrap();
    info!("Create output dataset");
    let mut output_ds = driver.create_with_band_type::<U>(output_file, out_metas.cols as isize, out_metas.rows as isize, out_count).unwrap();

    output_ds.set_geo_transform(&out_metas.geo_transform).unwrap();
    ...
    for f in &files {
        ...
        for i in &band_index {
            ...
            let src_data = src_band.read_as::<U>(
                src_window.position,
                src_window.size,
                dst_window.size,
